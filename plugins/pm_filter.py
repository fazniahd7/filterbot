    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                dulink =f"https://du-link.in/st?api=76e122f0034f858ae5747331f4165857502aba85&url=https://telegram.me/{temp.U_NAME}?start={ident}_{file_id}"
                print(dulink)
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"[{get_size(file.file_size)}] {file.file_name}", 
                    url=f"https://du-link.in/st?api=76e122f0034f858ae5747331f4165857502aba85&url=https://telegram.me/{temp.U_NAME}?start=pre_{file.file_id}"
                ),
            ]
            for file in files
        ]
